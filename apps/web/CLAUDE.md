...
## The call (M10, §25.3) — `src/components/call/`

§25.3's screen 17 over the §34.6 call socket. What is worth knowing:

- **`src/components/call/` is NOT the component library**, same rule as `today/` and `ask/`. M10 added zero components to `src/components/ui` — the screen composes `TaraPresence` (size `full`), `CallControls`, `Chip`, `Sheet` and `IconButton`.
...
- **The scrim is a SIBLING layer, never a filter on her** (§29.4: no filtering beyond the graded masters). But §25.3's literal "dimmed 25%" is **not implemented as a flat 25%, because the token layer cannot express a translucent overlay at all.** Every colour is a `var(--color-…)` holding a hex string, and Tailwind v3 emits *no rule whatsoever* for an opacity modifier on one — `bg-brand-navy-deep/25` compiles to nothing. The dim is gradient bands from the solid token colour to transparent, top and bottom, which is stronger exactly where §25.3 says the dim is for ("behind controls") and leaves her face clear. **The same defect is live in `Modal` and `Sheet`, whose `bg-brand-navy-deep/60` scrims have never rendered either** — that is a token-layer fix (a scrim token, or channel-triplet colours with `<alpha-value>`) and it is tracked separately.
