...
Two sockets, both real. `WS /chat/session` is S18's — §34.6 over the actual §9 pipeline, and since M9 it also carries §25.4's voice notes. `WS /call/session` is M10's: §25.3's live call, said in the same fifteen members. §33.5 still gates whether calls SHIP, and that gate is evaluated where a call is GRANTED (`POST /v1/call/session`), not in the socket.
...
- The full protocol behaviour (VAD ducking, barge-in, metering, degrade ladder) is M10's and lives in `call.py`.
...
## The call socket (M10, §25.3/§34.6) — invariants that must not regress
