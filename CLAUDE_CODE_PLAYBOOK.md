...
**Carried into M6 from M5-P6b (approved deferral):** diagram 8's nightly consolidation has three parts — *dedupe · decay stale · theme extraction*. **Decay shipped in M5** (`sitara_api.memory.decay`, pure arithmetic on a clock, no model needed). **Dedupe and theme extraction are M6's**, as a second Beat job beside the morning tick: both need the §32.5 embedding space and an LLM, which is exactly what M6 stands up. Dedupe merges near-duplicate memories within a user (cosine over the same 1024-d vectors, consent history preserved on the survivor); theme extraction is the consolidation half of §32.4. Neither may delete a memory the user consented to — §32.4 retains "until user deletes", so a merge keeps one row rather than removing two.
