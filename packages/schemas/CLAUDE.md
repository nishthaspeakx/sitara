...
- **A payload is typed by the milestone that EMITS it.** S18 typed the text-chat members; M9 typed `vad.state` and `tts.*` when voice notes started sending them. `barge_in` and `entitlement.warning` stay untyped — they belong to live calls, which §33.5 gates and M10 owns. `test_a_payload_is_typed_by_the_milestone_that_emits_it` holds the line in both directions, so M9's shapes cannot be dropped either.
