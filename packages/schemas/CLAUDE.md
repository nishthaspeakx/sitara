...
- **A payload is typed by the milestone that EMITS it.** S18 typed the text-chat members; M9 typed `vad.state` and `tts.*` when voice notes started sending them; **M9-P10b typed the last two, `barge_in` and `entitlement.warning`**, when the live call started sending them. All fifteen members now carry a shape, so the "not yet" half of `test_a_payload_is_typed_by_the_milestone_that_emits_it` is empty and what remains is the half that outlives the rule: none of them may be dropped.
- **M9-P10b added NO member, and that was the real test of §34.6.** The milestone that would most plausibly have needed a sixteenth — the first to stream audio, meter minutes and duck a speaker — said all three in members that already existed. The set stays closed at fifteen (§31.3).
- **`TtsStartPayload.tts_audio_asset_id` is OPTIONAL, and the null is load-bearing.** A voice note is synthesised whole and stored, so it has an asset and a replayable bubble. A live call is streamed and its audio is never stored (§13/§33.1) — so there is no asset, and null is the type saying exactly that rather than a field somebody forgot to fill.
- **`call-media.json` is the first set to land here BEFORE the two sides disagreed.** It is the internal socket between `sitara-realtime` and `sitara-api`, and it is here because the package's own rule says so: if both sides of a wire name a set, it belongs here before either side reads it. Deliberately NOT §34.6 — that set is closed and user-facing, and saying an internal transport in it would mean either widening a closed set or pretending an internal frame was a user-facing event. Python only, for the same reason `facts.py` and `cache_keys.py` are: `apps/web` never speaks it, and a TypeScript mirror no consumer reads is how a declared contract goes stale unnoticed.
- **`PartialCaptionPayload.role` is the constant `user`, not a `ChatRole`.** §34.6 forbids a partial caption of Tara's words (§9 runs grounding, language-quality and safety-post AFTER generation). Through M8 that held because nobody wrote the frame; M9 writes it, for the user's own speech, so the guarantee had to move into the type. This is the only use of the generator's `const:` type, and it should stay rare.
- **`voice.json` is the fourth set to land here after the two languages disagreed** — `services/api` wrote `transcript_status: "not_applicable"` / `playback_policy: "text_only"` while `apps/web` declared `"ready" | "pending" | "failed" | "none"`. Same story as the confidence states, the presence states and the memory types: invisible because no screen consumed it yet.
