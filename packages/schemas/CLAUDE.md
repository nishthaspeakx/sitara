...
- **A payload is typed by the milestone that EMITS it.** S18 typed the text-chat members; M9 typed `vad.state` and `tts.*` when voice notes started sending them; **M10 typed the last two, `barge_in` and `entitlement.warning`**, when the live call started sending them. All fifteen members now carry a shape, so the "not yet" half of `test_a_payload_is_typed_by_the_milestone_that_emits_it` is empty and what remains is the half that outlives the rule: none of them may be dropped.
- **M10 added NO member, and that was the real test of §34.6.** The milestone that would most plausibly have needed a sixteenth — the first to stream audio, meter minutes and duck a speaker — said all three in members that already existed. The set stays closed at fifteen (§31.3).
